...
# universal logger
log = logger.get_logger("parameters.py")
...
def concat(**args):
    ...
    mut_fig = make_subplots(rows=len(traces), cols=1, subplot_titles=names)
    for i, t in enumerate(traces):
        mut_fig.append_trace(t, i + 1, 1)
    mut_fig.update_layout(height=200 * len(traces))
    plotly.offline.plot(mut_fig, filename="test.html", auto_open=False)
