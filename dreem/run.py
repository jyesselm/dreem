...
@optgroup.option(
    "--map-overwrite", is_flag=True, help="overwrite mapping calculation"
)
@optgroup.option(
    "--skip",
    is_flag=True,
...
@optgroup.option("--tg_q_cutoff", default=None, help="TODO")
@optgroup.option("--bt2_alignment_args", default=None, help="TODO")
...
@optgroup.option(
    "--bv-overwrite", is_flag=True, help="overwrite bit vector calculation"
)
@optgroup.option(
    "--qscore_cutoff",
    default=None,
...
@optgroup.option(
    "--num_of_surbases", default=None, help="number of bases around a mutation"
)
@optgroup.option(
    "--map_score_cutoff",
    default=None,
...
@optgroup.option("--summary_output_only", is_flag=True, help="")
@optgroup.option("--plot_sequence", is_flag=True, help="")
def main(**args):
    """
...
def get_default_run_args():
    # generated by doing print(args)
    args = {
        "fasta": "",
        "fastq1": "",
        "fastq2": None,
...
def log_args_that_diff(args):
    d_args = get_default_run_args()
    log.info("ran with args: ")
    log.info(f"fa : {args['fasta']}")
    log.info(f"fq1: {args['fastq1']}")
    log.info(f"fq2 : {args['fastq2']}")
    for k, v in args.items():
        if k == "fasta" or k == "fastq1" or k == "fastq2":
            continue
        if v == d_args[k]:
            continue
        log.info(f"{k} : {v}")
